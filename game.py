...
class MyGame(object):
    ...
    def __init__(self, player_name, min_val, max_val, trials):
        ...
        for i in range(9):
            newinserters=[]
            for j in range(9):
                inserter = thorpy.Inserter()
                inserter.set_size((40,40))
                self.inserters.append(inserter)
                newinserters.append(inserter)
            newinserters = thorpy.make_group(newinserters)
            self.elements.append(newinserters)


        self.e_background = thorpy.Background( color=(200, 200, 255),
                                                    elements=self.elements)
        thorpy.store(self.e_background, gap=10)
    ...
    def reac_insert_func(self): #here is all the dynamics of the game
        print(("+" + "-"*9)*9 + "+")
        for count, i in enumerate(self.inserters):
            i.set_value("".join(
                sorted(
                    set(
                        [j for j in i.get_value() 
                         if j in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]
                         ]
                        )
                    )
                ))
            print("|" + i.get_value().center(9), end="")
            if (count+1)%9==0:
                print("|")
                print(("+" + "-"*9)*9 + "+")
        print()
